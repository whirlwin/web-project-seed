...
function createHttpBackend($browser, XHR, $browserDefer, callbacks, rawDocument, locationProtocol) {
  ...
  function jsonpReq(url, done) {
    ...
    // - fetches local customScripts via XHR and evals them
 ...
 * @example Example that demostrates basic bindings via ngClass directive.
   ...
   Therefore, if any CSS3 Transition/Animation customStyles (outside of ngAnimate) are set on the element, then, if a ngClass animation
 ...
 * for a manual update. The example is shown in two different declaration customStyles you may use
 ...
 * specificity (when !important isn't used with any conflicting customStyles). If a developer chooses to override the
 * styling to change how to hide an element then it is just a matter of using !important in their own CSS code.
 *
 * ### Overriding .ng-hide
 *
 * If you wish to change the hide behavior with ngShow/ngHide then this can be achieved by
 * restating the customStyles for the .ng-hide class in CSS:
 * <pre>
 * .ng-hide {
    ...
    compile: function(element, attr) {
      if (attr.type == 'text/ng-template') {
        var templateUrl = attr.id,
            // IE is not consistent, in customScripts we have to read .text but in other nodes we have to read .textContent
            text = element[0].text;
